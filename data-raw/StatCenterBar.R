...
# This contains the generation of `StatCenterBar2` which is required for `stat_centerBar2` to work. This is a new stat for ggplot2 that creates a barplot or line segments with bars centered around the x-axis. This is useful for creating matrix barplots. The `compute_group` function is used to compute the line segment endpoints.

StatCenterBar2 <- ggproto("StatCenterBar2", Stat,
                           compute_group = function(data, scales) {
                             ## Compute the line segment endpoints
                             x <- data$x
                             xend <- -data$x
...
usethis::use_data(StatCenterBar2, overwrite = TRUE)
